...
export class LetterMover {
    ...
    updateCanvas(canvasID, fewColors) {
        const canvas = document.querySelector(canvasID);
        if (canvas) {
            this.styleChanger.cssIDGradient(canvasID, fewColors);
        } else {
            console.error(`No element with the id "${canvasID}" found`);
        }
    }
    ...
    initialize() {
        ...
        setInterval(() => {
            ...
            this.updateCanvas(this.canvasID, this.fewColors);
        }, this.MOVE_INTERVAL);
        setInterval(
            () => this.updateCanvas(this.canvasID, this.fewColors),
            this.CANVAS_UPDATE_INTERVAL
        );
    }
}
