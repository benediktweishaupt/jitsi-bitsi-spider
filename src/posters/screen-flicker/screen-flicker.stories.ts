import type { Meta, StoryObj } from '@storybook/html';
import type { Speaker } from '../../types/speaker';
import { createScreenFlicker } from './screen-flicker';
import { speakers } from '../../data/speakers';

interface ScreenFlickerArgs {
  speaker: Speaker;
  speed: number;
}

const meta: Meta<ScreenFlickerArgs> = {
  title: 'Posters/ScreenFlicker',
  tags: ['autodocs'],
  argTypes: {
    speaker: {
      control: 'select',
      options: speakers.map((s) => s.name),
      mapping: Object.fromEntries(speakers.map((s) => [s.name, s])),
    },
    speed: {
      control: { type: 'range', min: 0.25, max: 4, step: 0.25 },
    },
  },
  args: {
    speaker: speakers[4], // Stefan Marx
    speed: 1,
  },
  render: (args) => {
    const container = document.createElement('div');
    const cleanup = createScreenFlicker(container, args.speaker, { speed: args.speed });
    (container as any).__cleanup = cleanup;
    return container;
  },
};

export default meta;
type Story = StoryObj<ScreenFlickerArgs>;

export const Default: Story = {};
